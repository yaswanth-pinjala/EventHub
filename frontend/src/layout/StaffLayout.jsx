  ...
  const menuItem = (path, label) => (
    <li
      className={`px-3 py-2 rounded-lg cursor-pointer ${
        location.pathname.includes(`/staff/${path}`) ||
        (path === "dashboard" && location.pathname === "/staff")
      ...
      <aside className="w-64 bg-white shadow-lg flex flex-col">
        ...
        <nav className="flex-1 px-2 py-4 text-sm">
          ...
          <ul className="space-y-1">
            ...
            {menuItem("upcoming-events", "Upcoming Events")}
            {menuItem("projects", "Academic Projects")}
          </ul>
        ...
        >
          ...
          >
            ...
            <div className="text-left">
              <p className="font-semibold">{staff.staffId}</p>
              <p className="text-gray-500 text-xs">{staff.name}</p>
            </div>
          </div>
